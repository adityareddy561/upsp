...
function loadUserById() {
	...
	$.ajax({
		...
		success: function (data) {
			...
			$('#name-input').val(data.data.fullName);
			$('#email').val(data.data.email);
			$('#mobileNumber').val(data.data.mobileNumber);
			
			$('#changepswd').html('<button class="editProfile" onclick="changePassword(' + data.data.id + ')">'
				+ 'Change Password</button>');
			$('#updtPswd').append('<button class="submit" onclick="updatePassword(' + data.data.id + ')" style="padding: 5px 10px"'
